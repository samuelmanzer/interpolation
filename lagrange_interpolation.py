...
def plot_lagrange_polys(x,lagrange_polys,y_exact,ax):
    ...
    return mae_list,rmsd_list


# Sets up a bar graph comparing the MAE and RMSD for different numbers
# of interpolation points
def plot_stats(mae_list,rmsd_list,n_points_range,ax):
    ind = np.arange(1,len(mae_list)+1)
    width = 0.5
    mae_rects = ax.bar(ind,mae_list,width,align="center",color='r')
    rmsd_rects = ax.bar(ind+width,rmsd_list,width,align="center",color='y')

    texts = []
    rect_idx = 0
    for rect in mae_rects+rmsd_rects:
...
mae_list,rmsd_list = plot_lagrange_polys(x,evenly_spaced_polys,y_exact,ax1)
texts_1 = plot_stats(mae_list,rmsd_list,n_points_range,ax3)
...
mae_list,rmsd_list = plot_lagrange_polys(x,chebyshev_point_polys,y_exact,ax2)
texts_2 = plot_stats(mae_list,rmsd_list,n_points_range,ax4)
